...
_ZoneManager.prototype._rayPickObjects = function (rayCaster, objects = null) {
    var inters = rayCaster.intersectObjects(objects || this.colObjects, true);
    if (inters.length > 0) {
        return inters[0];
    }
    return null;
...
_ZoneManager.prototype.objectRayPick = function (rayCaster, objects = null) {
    var objPickInfo = this._rayPickObjects(rayCaster, objects);
    if (objPickInfo) {
 ...
 * @returns {ItemObject}
 */
_ZoneManager.prototype.findNearestDropItem = function () {
    const refPos = MC.position;
    /** @type {ItemObject[]} */
    const itemObjects = this.objects.getItemObjects();

    let nearest = null;
    let distance = null;

    for (let item of itemObjects) {
        const newDistance = Math.abs(refPos.x - item.position.x) + Math.abs(refPos.y - item.position.y);

        if (!nearest || newDistance < distance) {
            nearest = item;
    ...
    // const nearest = this.objectRayPick(raycaster, itemObjects.map(o => o.pawn.rootObj));
    //
